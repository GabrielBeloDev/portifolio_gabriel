"use client";

import { useEffect, useState } from "react";

async function requestViewCount(
  slug: string,
  method: "GET" | "POST",
): Promise<number> {
  const response = await fetch(`/api/views/${encodeURIComponent(slug)}`, {
    method,
    cache: "no-store",
  });
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const payload = (await response.json()) as { count: number };
  return payload.count;
}

const viewsLabel = (count: number) =>
  count === 1 ? "1 leitura" : `${count} leituras`;

export function ViewTracker({ slug }: { slug: string }) {
  const [count, setCount] = useState<number | null>(null);

  useEffect(() => {
    let cancelled = false;
    const sessionKey = `viewed:${slug}`;
    const alreadyViewed = sessionStorage.getItem(sessionKey) !== null;
    // Marked before the request resolves so a re-run (strict mode, remount)
    // never fires a second increment for the same session
    if (!alreadyViewed) sessionStorage.setItem(sessionKey, "1");

    requestViewCount(slug, alreadyViewed ? "GET" : "POST")
      .then((total) => {
        if (!cancelled) setCount(total);
      })
      .catch((error: unknown) => {
        // Counter is an enhancement: log loud, render nothing
        console.error("view tracking failed", error);
      });

    return () => {
      cancelled = true;
    };
  }, [slug]);

  return (
    <span className="inline-block min-w-[12ch] font-mono">
      {count !== null && ` · ${viewsLabel(count)}`}
    </span>
  );
}
